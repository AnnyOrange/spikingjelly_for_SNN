...
from TET.TETLoss import TETLoss
from Exception import TrianerNotLoaded
from spikingjelly.clock_driven import functional


class Trainer:
    def __init__(self, name: str, net, train_data, validate_data, test_data, criterion: str = "MSELoss",
    ...
    def trian(self, total_epoch: int, categories, log_report, save_path: str = '', save_step: int = 100,
        ...
        if self.lr_scheduler_name == 'StepLR':
            lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer=self.optimizer, step_size=lr_scheuler_arg['step_size'])
        elif self.lr_scheduler_name == 'CosALR':
            lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=self.optimizer, T_max=lr_scheuler_arg['T_max'])
        else:
            lr_scheduler = None
        ...
        for epoch in range(self.start_epoch + 1, total_epoch + 1):
            ...
            for frame, label in self.train_data:
                ...
                if self.enable_amp:
                    with amp.autocast():
                        out = self.net(frame)
                        if self.criterion_name != "TETLoss":
                            out = out.mean(0)

                        if self.criterion_name == 'CrossEntropyLoss' or self.criterion_name == "TETLoss":
            ...
            intermediate_reporter(
                {'default': test_acc, 'train_loss': train_loss, 'train_acc': train_acc, 'test_loss': test_loss,
                 'test_acc': test_acc})
            if test_acc > max_test_acc and save_path is not '':
